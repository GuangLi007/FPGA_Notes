...
代码本质是在**扮演 595 的主机**，每个时钟周期对应 595 内部的一步动作：

| 代码 cnt | sh_cp (SRCLK) | ds (SER) | 595 内部发生了什么 |
|----------|---------------|----------|-------------------|
| 0 (空闲) | 0 | — | 595 时钟静默，输出保持 |
| 1 | 1→0 (上升沿) | b15 | 595 移位寄存器：`[0,0...0,b15]`，bit7→QH' |
| 2 | 0 | b14 | 595 无动作，FPGA 准备下一位 |
| 3 | 1→0 (上升沿) | b14 | 595 移位：`[0,0...b14,b15]` |
| ... | ... | ... | ... |
| 31 | 1→0 (上升沿) | b0 | 595 移位完成：两片 595 各 8 bit 都到位 |
| 32 | 0 | — | 保持，不移新数据（防 r_data 移空后 SER 误拉低） |
| 33 | — | — | `st_cp=1` (RCLK↑) → 595 存储寄存器更新，QA~QH **同时**输出 |
...
SER(ds) ─────────→│ 移位寄存器  │──→ QH' (级联)
                 ...
                 └──────┬─────┘
                        │ (cnt=33, RCLK↑)
                   ┌────▼──────┐
RCLK(st_cp) ───────→│ 存储寄存器  │──→ QA~QH (并行输出)
                   └───────────┘
```

移位寄存器在 SCLK 上升沿逐位右移，存储寄存器在 RCLK 上升沿一次性锁存——这就是"**串入并出 + 锁存防抖动**"的硬件原理。
...
- 工程记录：[[24Segment_Display_1_工程记录与思路]]
